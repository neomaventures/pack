...
export default tseslint.config(
  {
    ignores: [
      ...
      "**/pnpm-workspace.yaml",
      // TypeORM migration files — generated by the typeorm CLI from a
      // separate tsconfig (`templates/saas/typeorm/tsconfig.json`) and not
      // part of the root TS project service.
      "**/typeorm/migrations/**",
    ],
  },
  ...
  eslintPluginPrettierRecommended,
  {
    languageOptions: {
      ...
      parserOptions: {
        ...
        // `packages/*` (uses this config) plus `templates/saas` (uses its own
        // `eslint.config.mjs`). Each config must anchor its own root.
  ...
  },
  {
    ...
    rules: {
      ...
      // Barrel-only export tree (#28): historically forbade deep subpath
      // imports via `@neomaventures/request-context/*`. request-context now
      // ships a legitimate `/testing` subpath, so the restriction is lifted.
      // Any future package-internal subpath restrictions live in that
      // package's own eslint override.
