import { Global, Inject, Injectable, Module, type Type } from "@nestjs/common"
import { Test } from "@nestjs/testing"
import { getDataSourceToken } from "@nestjs/typeorm"
import { type DataSource } from "typeorm"

import { Post } from "./post.entity"
import { createTestDbModule } from "./test-db.module"
...
describe("createTestDbModule", () => {
  describe("when imported into a Test module", () => {
    ...
    it("registers only the entities passed to the factory", async () => {
      const module = await Test.createTestingModule({
        imports: [createTestDbModule([Post])],
      }).compile()
  ...
  describe("global scope", () => {
    it("exposes the datasource to downstream global modules without re-import", async () => {
      @Injectable()
      ...
      const module = await Test.createTestingModule({
        imports: [createTestDbModule([User]), InjectingModule, ConsumerModule],
      }).compile()
  ...
  describe("per-test teardown", () => {
    ...
    it("returns a destroyed datasource in the next test", async () => {
  ...
  describe("isolation between factory calls", () => {
    it("returns a distinct module class on each call", () => {
      const first: Type<unknown> = createTestDbModule([User])
      const second: Type<unknown> = createTestDbModule([User])
