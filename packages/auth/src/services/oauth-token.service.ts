 ...
 * slot that backs `getPrincipal()` and `@Principal()`) and inspects its
 * eagerly-loaded `oauthTokens` relation, filtering for the requested
 * provider. Returns a sanitised snapshot — `refreshToken` is
...
export class OAuthTokenService {
   ...
   * or `null` when no usable token exists.
   *
