 ...
 * `(account, provider)` pair — and the account exposes them via a
 ...
 *   instead of full-row UPDATEs on the account.
 * - Indexes (`(accountId, provider)`) and per-column types
 ...
 *   public email!: string
 *
 *   @OneToMany(() => OAuthToken, (t) => t.account, {
 *     eager: true,
 *     cascade: false,
 *   })
