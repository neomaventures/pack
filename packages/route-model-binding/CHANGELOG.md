...
- a1223cb: Drop the silent-null workaround. `RouteModelBindingMiddleware` now throws `NotFoundException` directly when an entity cannot be resolved, instead of stashing `null` on the request for the `@RouteModel()` decorator to throw on. The HTTP wire shape (`404`, message, `error: "Not Found"`) is unchanged.
